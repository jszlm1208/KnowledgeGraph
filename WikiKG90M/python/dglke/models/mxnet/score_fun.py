...
def batched_l2_dist(a, b):
    ...
    squared_res = nd.add(nd.linalg_gemm(
        ...
        nd.broadcast_axes(
            nd.expand_dims(
                b_squared, axis=-2), axis=1, size=a.shape[1]),
        alpha=-2),
                         nd.expand_dims(
                             a_squared, axis=-1))
...
class TransEScore(nn.Block):
    ...
    def edge_func(self, edges):
        ...
        score = head + rel - tail
        return {
            'score': self.gamma - nd.norm(
                score, ord=self.dist_ord, axis=-1)
        }
...
class TransRScore(nn.Block):
    ...
    def prepare(self, g, gpu_id, trace=False):
        ...
        head_emb = g.ndata['emb'][head_ids.as_in_context(g.ndata[
            'emb'].context)].expand_dims(axis=-2)
        tail_emb = g.ndata['emb'][tail_ids.as_in_context(g.ndata[
            'emb'].context)].expand_dims(axis=-2)
            ...
            def fn(rel_id, num_chunks, head, tail, gpu_id, trace=False):
                ...
                for i in range(num_chunks):
                    ...
                    for j in range(num_nnodes):
                        ...
                        head_neg = nd.broadcast_axis(
                            head_neg, axis=0, size=num_rels)
            ...
            def fn(rel_id, num_chunks, head, tail, gpu_id, trace=False):
                ...
                for i in range(num_chunks):
                    ...
                    for j in range(num_nnodes):
                        ...
                        tail_neg = nd.broadcast_axis(
                            tail_neg, axis=0, size=num_rels)
    ...
    def writeback_local_emb(self, idx):
        self.global_projection_emb.emb[
            idx] = self.projection_emb.emb.as_in_context(mx.cpu())[idx]
...
class ComplExScore(nn.Block):
    ...
    def edge_func(self, edges):
        real_head, img_head = nd.split(
            edges.src['emb'], num_outputs=2, axis=-1)
        real_tail, img_tail = nd.split(
            edges.dst['emb'], num_outputs=2, axis=-1)
    ...
    def create_neg(self, neg_head):
        if neg_head:
            ...
            def fn(heads, relations, tails, num_chunks, chunk_size,
                ...
                relations = relations.reshape(-1, self.relation_dim,
                                              self.entity_dim)
...
class RotatEScore(nn.Block):
    ...
    def edge_func(self, edges):
        ...
        #sqrt((x*x).sum() + eps)
        score = mx.nd.sqrt(real_score * real_score + img_score * img_score +
                           self.eps).sum(-1)
    ...
    def infer(self, head_emb, rel_emb, tail_emb):
        ...
        real_score = re_head.expand_dims(axis=1) * re_rel.expand_dims(
            axis=0) - im_head.expand_dims(axis=1) * im_rel.expand_dims(axis=0)
        img_score = re_head.expand_dims(axis=1) * im_rel.expand_dims(
            axis=0) + im_head.expand_dims(axis=1) * re_rel.expand_dims(axis=0)

        real_score = real_score.expand_dims(axis=2) - re_tail.expand_dims(
            axis=0).expand_dims(axis=0)
    ...
    def create_neg(self, neg_head):
        ...
        if neg_head:
            ...
            def fn(heads, relations, tails, num_chunks, chunk_size,
                ...
                score = mx.nd.sqrt(score_real * score_real + score_img *
                                   score_img + self.eps).sum(-1)
